...
import { toLocalDateTimeTable } from '../../../../utils/date';
...
export class EventListComponent {
  ...
  columnDefs: ColDef<Event>[] = [
    ...
    { headerName: 'Título', field: 'title', sortable: true, filter: true },
    {
      ...
      valueFormatter: (params) => {
        ...
        return toLocalDateTimeTable(new Date(params.value));
      },
    },
    {
      headerName: 'Fin',
    ...
    },
    {
      ...
      cellRendererParams: {
        ...
        actions: [
          ...
          { label: 'Presupuesto', icon: 'attach_money', action: 'ACCOUNT' },
  ...
  onGridSizeChanged(params: GridSizeChangedEvent): void {
    ...
    const showColumns =
      width < 768 ? ['title', 'startDate', 'actions'] : this.displayedColumns;
  ...
  eventAccount(event: Event): void {
    console.log('ruta', event.account.idAccount);
    this.router.navigate(['events/account', event.account.idAccount]);
  }
  ...
  changeEventStatusByContext(
    ...
    this.alertService
      .selectOption<EventStatus>(
        ...
        `Seleccione el nuevo estado del evento: ${
          event.title
        } con fecha de inicio ${new Date(
          event.startDate
        ).toLocaleDateString()} y fin ${new Date(
          event.endDate
        ).toLocaleDateString()}.`,
        statusOptions
      )
