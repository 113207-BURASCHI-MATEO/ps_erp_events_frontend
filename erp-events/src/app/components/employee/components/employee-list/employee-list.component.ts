...
export class EmployeeListComponent implements OnInit {
    ...
    this.alertService.delete('este empleado').then((confirmed) => {
      if (confirmed) {
        this.employeeService.delete(id).subscribe({
          ...
          error: (err) => {
            this.alertService.showErrorToast(
              `Error al eliminar empleado ${err.error.readableMessage}`
            );
          },
        });
      }
  ...
  renderFile(employee: Employee): void {
    this.router.navigate(['/files/entity', 'employee', employee.idEmployee]);
  }
  ...
  exportToExcel(): void {
    this.exportService.exportToExcel(this.employees, 'Empleados', (emp) => ({
      ID: emp.idEmployee,
